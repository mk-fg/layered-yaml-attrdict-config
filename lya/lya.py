    ...
    def construct_mapping(self, node, deep=False):
        if isinstance(node, yaml.MappingNode):
            ...
            raise yaml.constructor.ConstructorError(
                None, None, 'expected a mapping node, but found {}'.format(node.id),
                node.start_mark)

        mapping = OrderedDict()
        for key_node, value_node in node.value:
            # default is to not recurse into keys
...
class AttrDict(OrderedDict):
    ...
    def from_yaml(cls, path_or_file, if_exists=False):
        src_load = lambda src: cls.from_data(
            yaml.load(src, OrderedDictYAMLLoader))
    ...
    def _lya__apply(self, func, items=False, vals_only=True, update=True):
        for k, v in self.items():
            ...
            if v_is_dict:
                v._lya__apply(
                    func, items=items, vals_only=vals_only, update=update)
    ...
    def _lya__apply_flat(self, func, update=True):
        ...
        for n, (k, v) in enumerate(flat):
            ...
            if update:
                flat[n] = k, v
        if update:
            self.update_flat(flat)

    def _lya__filter(self, func, items=False):
        for k, v in self.items():
            if isinstance(v, AttrDict):
                v._lya__filter(func, items=items)
            else:
                v = func(v) if not items else func(k, v)
                if not v:
                    del self[k]
...
def configure_logging(cfg, custom_level=None, debug=_no_arg):
    ...
    if debug is not _no_arg:
        if custom_level is not None:
            raise ValueError((
                'Either "custom_level" ({!r}) or "debug"'
